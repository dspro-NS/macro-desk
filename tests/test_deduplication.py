...
from macro_desk.ingestion.sources import DOCUMENT_TYPE_NOTIFICATION, DOCUMENT_TYPE_PRESS_RELEASE
...
def test_configured_feeds_ingest_press_releases_and_notifications(settings, repository: DocumentRepository) -> None:
    ...
    notifications = repository.list_newest(document_type=DOCUMENT_TYPE_NOTIFICATION)
    assert [item.title for item in notifications] == ["Master Direction on KYC"]
